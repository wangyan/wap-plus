...
function catType($catid)
{
		...
		return $catType = $category[$catid]['type'];
}

/**
 * 通过分类ID获取栏目类型
 */
function catType2($typeid)
{
	$type_arr = getcache('wap_type','wap');
	$catId = $type_arr[$typeid][cat];
	return catType($catId);
}
 ...
 * $id 文章id
 */
function page_url($catid, $typeid='') {
	global $WAP;
	if($typeid=='') {
		$types = getcache('wap_type','wap');
		foreach ($types as $type) {
    ...
    return WAP_SITEURL."&amp;a=lists&amp;catid=$catid&amp;typeid=$typeid";
}
?>